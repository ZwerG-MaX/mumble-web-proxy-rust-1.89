...
use openssl::ssl::{ConnectConfiguration, SslAcceptorBuilder, Ssl};
...
impl<S: AsyncRead + AsyncWrite + Send + Unpin> DtlsBuilder<S> for ConnectConfiguration {
        ...
        S: 'async_trait,
    {
        ...
        let ssl = self
            .into_ssl("invalid")
            .map_err(|_| io::Error::other("SSL configuration error"))?;
        let mut stream = SslStream::new(ssl, stream.compat())
            .map_err(|_| io::Error::other("SSL stream error"))?;
        Pin::new(&mut stream)
            .connect()
            .await
            .map_err(|_| io::Error::other("handshake error"))?;
        Ok(stream.compat())
    }
}
