...
impl Srtp {
    ...
    fn estimate_packet_index(context: &Self, seq_num: u16) -> U48 {
        ...
        } else if highest_seq_num - mid_seq_num > seq_num {
...
impl Protocol for Srtp {
    ...
    fn get_authenticated_bytes<'a>(
        ...
        track_try!(auth_bytes.write_u32be(roc));
    ...
    u64: From<P::PacketIndex>,
{
    ...
    pub fn decrypt_portion(
        ...
        let iv = iv ^ (BigUint::from(index) << 16usize);
...
fn hmac_hash_sha1(key: &[u8], data: &[u8]) -> Vec<u8> {
    let mut hmac = hmac::Hmac::<sha1::Sha1>::new_from_slice(key).expect("Correct Key Length");
    hmac.update(data);
...
fn prf_n(master_key: &[u8], x: BigUint, n: usize) -> Vec<u8> {
    ...
    let mut ctr = aes_ctr::Aes128Ctr::new_var(master_key, &(x << 16usize).to_bytes_be())
...
pub(crate) mod test {
    ...
    fn rtp_decryption_works() {
        ...
        let mut packet = TEST_SRTP_PACKET;
    ...
    fn rtp_decryption_with_rollover_works() {
        ...
        let mut rtp_reader = SrtpPacketReader::new(context, rfc3550::RtpPacketReader);
        let mut packet_before = TEST_2_SRTP_PACKET_BEFORE_ROLLOVER;
        rtp_reader.read_packet(&mut packet_before).unwrap();
        let mut packet_after = TEST_2_SRTP_PACKET_AFTER_ROLLOVER;
        rtp_reader.read_packet(&mut packet_after).unwrap();
    }
