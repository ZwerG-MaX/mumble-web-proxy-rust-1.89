...
impl WriteTo for RtcpSenderReport {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        track_try!(payload.write_u32be(self.ssrc));
        track_try!(payload.write_u64be(self.ntp_timestamp));
        track_try!(payload.write_u32be(self.rtp_timestamp));
        track_try!(payload.write_u32be(self.sent_packets));
        track_try!(payload.write_u32be(self.sent_octets));
...
impl Default for RtcpSourceDescription {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ReadFrom for SdesChunk {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        track_try!(reader.read_bytes(padding_len)); // discard
...
impl WriteTo for RtcpGoodbye {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        for x in self.ssrc_csrc_list.iter() {
            track_try!(payload.write_u32be(*x));
        }
        if let Some(ref reason) = self.reason {
            track_assert!(reason.len() <= 0xFF, ErrorKind::Invalid);
            track_try!(payload.write_u8(reason.len() as u8));
            track_try!(payload.write_all(reason.as_bytes()));

            let padding_len = (4 - (reason.len() + 1) % 4) % 4;
            for _ in 0..padding_len {
                track_try!(payload.write_u8(0));
            }
        }
...
impl WriteTo for RtcpApplicationDefined {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        let mut payload = Vec::new();
        track_try!(payload.write_u32be(self.ssrc_or_csrc));
